...
//after the 16 tables
  int *p=pg_tbl[15]+1024;

//USRSTART >> 12 >> 10 = 256
  pg_dir[256]= (int)p | PTE_P | PTE_W | PTE_U;

//page table (100000 >> 12 = 256)
  for(i=0; i<256; i++)
    p[i]= (USRPHY + (i<<12) ) | PTE_P | PTE_W | PTE_U;
}
...
//mapping (kernstart >> 12 >> 10 = 3<<8)
  for(i=0; i<1024; i++)
    if (i>=(3<<8) && i<(3<<8)+16)
      pg_dir[i]= (int)pg_tbl[i-(3<<8)] | PTE_W | PTE_P;
    else
      pg_dir[i]=0;
