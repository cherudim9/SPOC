...
class fs:
    ...
    # DONE
        ...
        parent_Inum = self.nameToInum[self.getParent(tfile)]
        parent_addr = self.inodes[parent_Inum].getAddr()
    ...
    def createLink(self, target, newfile, parent):
        # add new file (pointed to target) under parent
        parent_Inum = self.nameToInum[parent]
        parent_addr = self.inodes[parent_Inum].getAddr()
        if self.data[parent_addr].getFreeEntries() == 0:
            return -1;
        if self.data[parent_addr].dirEntryExists(newfile):
            return -1;
        ...
        self.data[parent_addr].addDirEntry(newfile, target_Inum)
    ...
    # DONE
        ...
        if self.data[parent_addr].dirEntryExists(newfile):
            return -1;
        new_Inum=self.inodeAlloc()
        if new_Inum<0:
            return -1
        if ftype=='d':
            new_addr = self.dataAlloc()
            if new_addr<0:
                self.inodeFree(new_Inum)
                return -1
            self.data[new_addr].setType('d')
            self.data[new_addr].addDirEntry('.', new_Inum)
            self.data[new_addr].addDirEntry('..', parent_Inum)
            self.inodes[new_Inum].setAll('d', new_addr, 2) # 2 = parent + self
        if ftype=='f':
            self.inodes[new_Inum].setAll('f', -1, 1) # 1 = parent
        self.inodes[parent_Inum].incRefCnt()
        self.data[parent_addr].addDirEntry(newfile, new_Inum)
